...
from regras_if import (
    Config, classificar_por_heuristica, normalizar,
...
# --------------------------------------------------------------------------- #
# Reconhecimento flexível de nomes de coluna
# Cada nome canônico (usado internamente) aceita vários "sinônimos". A
# comparação é feita sem acento e em minúsculas, então "Conta Contábil",
# "conta contabil" e "CONTA CONTÁBIL" são tratados como iguais.
# Para aceitar um nome novo do seu balancete, basta acrescentá-lo na lista.
# --------------------------------------------------------------------------- #
_ALIASES_COMUNS = {
    "Conta": [
        "conta", "codigo", "codigo da conta", "cod conta", "cod. conta",
        "conta contabil", "cta", "codigo cosif", "conta cosif",
        "codigo contabil", "codigo da conta contabil", "numero da conta",
        "conta reduzida", "conta analitica", "codigo reduzido",
    ],
    "Descricao": [
        "descricao", "historico", "nome", "nome da conta", "titulo",
        "descricao da conta", "conta descricao", "nomenclatura",
        "titulo da conta", "conta titulo",
    ],
}
_ALIASES_BALANCETE = dict(_ALIASES_COMUNS)
_ALIASES_BALANCETE["Saldo_Atual"] = [
    "saldo_atual", "saldo atual", "saldo", "saldo final", "saldo atualizado",
    "saldo final do periodo", "saldo do periodo", "valor", "saldo r$",
    "saldo (r$)", "saldo atual (r$)", "vl saldo", "saldo liquido",
    "saldo atual da conta", "saldo contabil",
]
_ALIASES_CADASTRO = dict(_ALIASES_COMUNS)
_ALIASES_CADASTRO["Classificacao"] = [
    "classificacao", "classificacao pis/cofins", "classificacao pis cofins",
    "tratamento", "enquadramento", "classificacao tributaria", "classe",
    "tratamento pis/cofins", "tratamento tributario",
]
_ALIASES_CADASTRO["Natureza"] = ["natureza", "tipo", "natureza da conta", "grupo"]
_ALIASES_CADASTRO["CST_PIS"] = ["cst_pis", "cst pis", "cstpis"]
_ALIASES_CADASTRO["CST_COFINS"] = ["cst_cofins", "cst cofins", "cstcofins"]
_ALIASES_CADASTRO["Observacao"] = ["observacao", "obs", "observacoes", "nota", "notas"]


def _mapear_cabecalho(cabecalho: list, aliases: dict) -> dict:
    """
    Descobre qual coluna REAL do arquivo corresponde a cada nome canônico.
    Devolve {canonico: nome_real}. Se um canônico não for encontrado, ele
    simplesmente não aparece no resultado (aí é tratado como coluna ausente).
    """
    norm = {normalizar(h): h for h in cabecalho if h}   # cabeçalhos reais normalizados
    canon = {}
    for canonico, nomes in aliases.items():
        if normalizar(canonico) in norm:                # bate o próprio nome canônico
            canon[canonico] = norm[normalizar(canonico)]
            continue
        for nome in nomes:                              # tenta cada sinônimo
            if normalizar(nome) in norm:
                canon[canonico] = norm[normalizar(nome)]
                break
    return canon
...
def ler_balancete(caminho: str) -> List[dict]:
    ...
    canon = _mapear_cabecalho(cabecalho, _ALIASES_BALANCETE)
    # Obrigatórias: Conta e Saldo_Atual. Descrição é opcional (ajuda a heurística).
    faltando = [c for c in ("Conta", "Saldo_Atual") if c not in canon]
    if faltando:
        raise SystemExit(
            f"[ERRO] Balancete sem as colunas obrigatórias: {faltando}.\n"
            f"       Colunas encontradas no arquivo: {cabecalho}\n"
            f"       Renomeie a coluna correspondente (ou me informe o nome usado\n"
            f"       para eu incluí-lo na lista de sinônimos).")
    col_conta, col_saldo = canon["Conta"], canon["Saldo_Atual"]
    col_desc = canon.get("Descricao")
    linhas = []
    for d in registros:
        conta = str(d.get(col_conta) or "").strip()
        if conta == "":
            continue
        linhas.append({
            "Conta": conta,
            "Descricao": str(d.get(col_desc) or "").strip() if col_desc else "",
            "Saldo_Atual": _num(d.get(col_saldo)),
        })
    return linhas
...
def ler_cadastro(caminho: str) -> List[dict]:
    ...
    canon = _mapear_cabecalho(cabecalho, _ALIASES_CADASTRO)
    # Obrigatórias: Conta e Classificacao. As demais são opcionais.
    faltando = [c for c in ("Conta", "Classificacao") if c not in canon]
    if faltando:
        raise SystemExit(
    ...
    linhas = []
    for d in registros:
        conta = str(g(d, "Conta") or "").strip()
        if conta == "":
            continue
        linhas.append({
            "Conta": conta,
            "Descricao": str(g(d, "Descricao") or "").strip(),
            "Natureza": str(g(d, "Natureza") or "").strip().upper(),
            "Classificacao": str(g(d, "Classificacao") or "").strip().upper(),
            "CST_PIS": str(g(d, "CST_PIS") or "").strip(),
            "CST_COFINS": str(g(d, "CST_COFINS") or "").strip(),
            "Observacao": str(g(d, "Observacao") or "").strip(),
        })
    return linhas
